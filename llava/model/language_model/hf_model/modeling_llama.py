...
from .ms_poe import MsPoELlamaRotaryEmbedding
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None, enable_mda_attention=False, visual_full_available=False):  
    """
    ...
    if enable_mda_attention is False:
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.enable_mda_attention = config.enable_mda_attention
        self.visual_full_available = config.visual_full_available
        self._init_rope()
    ...
    def forward(
        ...
        if attention_mask is not None:
            if self.enable_mda_attention is False:
                ...
                if attention_mask[0].size() != (bsz, 1, q_len, kv_seq_len):
                    raise ValueError(
                        f"Attention mask should be of size {(bsz, 1, q_len, kv_seq_len)}, but is {attention_mask[0].size()}"
                    )
            if self.enable_mda_attention is False:
                ...
                text_only_indexes = torch.tensor([ (each <= torch.finfo(each.dtype).min).all() for each in attention_mask[0] ], dtype=torch.bool,device = attn_weights_img.device)
        ...
        if self.enable_mda_attention is False:
            ...
            attn_weights_img[text_only_indexes] = 0
...
class MsPoELlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.enable_ms_poe = config.enable_ms_poe
        if self.enable_ms_poe:
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            if self.enable_ms_poe:
                ...
                self.rotary_emb_ori = LlamaRotaryEmbedding(self.head_dim, max_position_embeddings=self.max_position_embeddings)
            ...
            scaling_type = self.config.rope_scaling["type"]
            scaling_factor = self.config.rope_scaling["factor"]
            assert self.enable_ms_poe!=True, "MS-POE is not supported with custom RoPE scaling"
            if scaling_type == "linear":
                self.rotary_emb = LlamaLinearScalingRotaryEmbedding(
                    self.head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=scaling_factor
                )
    ...
    def forward(
        ...
        if self.enable_ms_poe:
            ori_cos , ori_sin = self.rotary_emb_ori(value_states, seq_len=kv_seq_len)
            temp_query_states, temp_key_states = apply_rotary_pos_emb(query_states, key_states, ori_cos, ori_sin, position_ids)
            self.head_order = self._head_wise_statistics(temp_query_states, temp_key_states, q_len, kv_seq_len, bsz, attention_mask)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.enable_mda_attention = config.enable_mda_attention # this is new :)
        self.visual_full_available = config.visual_full_available # this is new :)
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        ...
        if attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            expanded_attn_mask = _expand_mask(attention_mask, inputs_embeds.dtype, tgt_len=input_shape[-1], enable_mda_attention=self.enable_mda_attention, visual_full_available=self.visual_full_available)  
            # .to(
            #     inputs_embeds.device
            # )

            if self.enable_mda_attention:
                ...
                if USE_FAST_V and USE_FAST_V_Token_Drop and past_key_value != None:
                    ...
                    if idx < AGG_LAYER:
                        ...
                        keep_indexs = torch.cat((torch.arange(SYS_LENGTH, device=device), top_attention_rank_index,
                                                 torch.arange(SYS_LENGTH + IMAGE_TOKEN_LENGTH,
                                                              past_key_value[0].shape[2], device=device)))
                        # sort index
                        ...
                        new_past_key_value = (
                        past_key_value[0][:, :, keep_indexs, :], past_key_value[1][:, :, keep_indexs, :])
